#!/usr/bin/env python3
"""Full eval: load LoRA adapter, run against HF Space, report scores."""
...
model = PeftModel.from_pretrained(model, "train_output/lora_adapter")
    ...
    for pattern in [r"\{[^{}]*\"action_type\"\s*:\s*\"[^\"]+?\"[^{}]*\}", r"```json\s*(\{.*?\})\s*```"]:
        ...
        if m:
            try:
                ...
                if "action_type" in parsed: return parsed
            except: continue
    return None
...
def run_eval(task, n=50):
    ...
    for _ in range(n):
        ...
        if parsed:
            json_ok += 1
            action = parsed
            # Ensure hypothesis from world_type
