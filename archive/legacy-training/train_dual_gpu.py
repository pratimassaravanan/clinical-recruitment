...
def run_eval(mdl, tok, task, n=50, device="cuda:0"):
    ...
    for _ in range(n):
        ...
        try:
            ...
            r = client.post(f"{ENV_URL}/step", json=action)
            r.raise_for_status()
            result = r.json(); obs = result; total_r += result.get("reward", 0); steps += 1
        except: break
    client.close()
...
for ep in range(RL_EPISODES):
    ...
    r = client.post(f"{ENV_URL}/reset", params={"task_id": task})
    r.raise_for_status()
    result = r.json(); obs = result; total_r, steps = 0.0, 0
    ...
    for step in range(30):
        ...
        r.raise_for_status()
        result = r.json(); obs = result; reward = result.get("reward", 0); total_r += reward; steps += 1
