...
Prerequisites:
    1. Generate traces locally first (fast, ~3 min for 5000):
       python scripts/generate_traces.py --num 5000 --threads 8 --output data/sft_traces_5k.json

    2. Install training extras:
       pip install unsloth "trl>=0.19.0" "transformers>=5.2.0,<=5.5.0" "datasets>=2.21.0" "accelerate>=0.34.0"

Usage:
    python train.py

    # Or with custom settings:
    SFT_EPOCHS=10 MODEL_NAME="unsloth/Qwen3-4B-unsloth-bnb-4bit" python train.py
"""
import os
...
MAX_SEQ      = int(os.getenv("MAX_SEQ", "1024"))   # 768 was too short, truncated complex observations
LORA_R       = int(os.getenv("LORA_R", "16"))
LORA_ALPHA   = int(os.getenv("LORA_ALPHA", "16"))
SFT_EPOCHS   = int(os.getenv("SFT_EPOCHS", "3"))   # 3 epochs over 4500+ traces is plenty
SFT_LR       = float(os.getenv("SFT_LR", "2e-4"))
SFT_BATCH    = int(os.getenv("SFT_BATCH", "1"))    # safe default for T4 16GB
SFT_GRAD_ACC = int(os.getenv("SFT_GRAD_ACC", "4"))  # effective batch = 1*4 = 4
...
for trace in all_traces:
    try:
        ...
        skipped += 1
        if skipped <= 3:
            print(f"  WARNING: Skipped trace (template error): {e}")
if skipped:
    print(f"  Skipped {skipped}/{len(all_traces)} traces due to template errors")
