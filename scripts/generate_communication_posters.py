...
PALETTE = {
    ...
    "panel": (18, 31, 52, 232),
    "ink": (245, 239, 228, 255),
    "ink_soft": (201, 210, 221, 255),
    "ink_faint": (141, 159, 182, 255),
    "teal": (99, 223, 213, 255),
...
def header(
    ...
    chip_box = (108, 96, 540, 152)
    rounded_panel(draw, chip_box, fill=(245, 239, 228, 20), outline=accent[:3] + (120,), radius=28, width=2, shadow=False)
    draw.text((132, 107), chip_label, font=chip_font, fill=PALETTE["ink_soft"])

    paragraph(draw, 112, 206, title, title_font, PALETTE["ink"], 960, gap=0)
    paragraph(draw, 116, 430, subtitle, sub_font, PALETTE["ink_soft"], 980, gap=10)
    draw.line((112, 520, 1092, 520), fill=accent[:3] + (140,), width=4)
    draw.line((112, 546, 668, 546), fill=(255, 255, 255, 36), width=2)
    ...
    draw.text((1290, 120), f"0{level}", font=num_font, fill=(245, 239, 228, 20))
...
def card(
    ...
    rounded_panel(draw, box, fill=PALETTE["panel"], outline=accent[:3] + (76,), radius=32, width=2)
    x1, y1, x2, y2 = box
    kick_font = font(DISPLAY, 26)
    title_font = font(DISPLAY, 40)
...
def stat_box(
    ...
    rounded_panel(draw, box, fill=(245, 239, 228, 18), outline=accent[:3] + (88,), radius=28, width=2)
    x1, y1, x2, y2 = box
...
def footer(
    ...
    for box, text in blocks:
        rounded_panel(draw, box, fill=(245, 239, 228, 14), outline=(255, 255, 255, 26), radius=24, width=2, shadow=False)
        paragraph(draw, box[0] + 22, box[1] + 24, text, foot_font, PALETTE["ink_soft"], box[2] - box[0] - 44, gap=4)
    draw.line((104, 2160, 1698, 2160), fill=accent[:3] + (120,), width=3)


def funnel_focus(draw: ImageDraw.ImageDraw, box: tuple[int, int, int, int], accent: tuple[int, int, int, int]) -> None:
    rounded_panel(draw, box, fill=PALETTE["panel"], outline=(255, 255, 255, 30), radius=38, width=2)
    x1, y1, x2, y2 = box
    cx = (x1 + x2) // 2
    cy = y1 + 630
    for radius, col, width in [
        (340, PALETTE["mist"][:3] + (90,), 4),
        (254, accent[:3] + (150,), 6),
        (178, PALETTE["copper"][:3] + (110,), 4),
    ]:
        draw.arc((cx - radius, cy - radius, cx + radius, cy + radius), -78, 252, fill=col, width=width)
    for deg in range(-78, 253, 15):
        rad = math.radians(deg)
        ox = cx + math.cos(rad) * 350
        oy = cy + math.sin(rad) * 350
        ix = cx + math.cos(rad) * 330
        iy = cy + math.sin(rad) * 330
    ...
    for text, lx, ly in labels:
        bx = x1 + lx
        by = y1 + ly
        rounded_panel(draw, (bx, by, bx + 210, by + 72), fill=(245, 239, 228, 16), outline=(255, 255, 255, 38), radius=22, width=2, shadow=False)
        draw.text((bx + 24, by + 18), text.upper(), font=label_font, fill=PALETTE["ink_soft"])
...
def phase_focus(draw: ImageDraw.ImageDraw, box: tuple[int, int, int, int], accent: tuple[int, int, int, int]) -> None:
    ...
    for idx, (name, desc) in enumerate(nodes):
        ...
        rounded_panel(draw, panel, fill=(245, 239, 228, 16), outline=(255, 255, 255, 30), radius=24, width=2, shadow=False)
...
def action_focus(draw: ImageDraw.ImageDraw, box: tuple[int, int, int, int], accent: tuple[int, int, int, int]) -> None:
    ...
    for (px, py), text in zip(coords, pills):
        rounded_panel(draw, (px, py, px + 230, py + 82), fill=(245, 239, 228, 16), outline=(255, 255, 255, 34), radius=22, width=2, shadow=False)
        paragraph(draw, px + 18, py + 20, text.upper(), pill_font, PALETTE["ink_soft"], 194, gap=4)
    paragraph(
        draw,
        x1 + 38,
        y2 - 220,
...
def results_focus(
    ...
    rounded_panel(draw, left_box, fill=PALETTE["panel"], outline=(255, 255, 255, 30), radius=38, width=2)
    rounded_panel(draw, right_box, fill=PALETTE["panel"], outline=(255, 255, 255, 30), radius=38, width=2)
    ...
    for label, ptext in pvals:
        draw.text((right_box[0] + 36, y), label, font=font(DISPLAY, 28), fill=PALETTE["ink"])
        draw.text((right_box[0] + 36, y + 44), ptext, font=font(BODY, 30), fill=PALETTE["ink_soft"])
        draw.line((right_box[0] + 36, y + 96, right_box[2] - 36, y + 96), fill=(255, 255, 255, 24), width=2)
        y += 136

    verdict = (right_box[0] + 36, right_box[1] + 980, right_box[2] - 36, right_box[1] + 1230)
    rounded_panel(draw, verdict, fill=(99, 223, 213, 20), outline=accent[:3] + (120,), radius=24, width=2, shadow=False)
    draw.text((verdict[0] + 26, verdict[1] + 24), "VERDICT", font=font(DISPLAY, 28), fill=PALETTE["ink_faint"])
    paragraph(draw, verdict[0] + 26, verdict[1] + 70, "No pairwise comparison reaches p < 0.05.", font(DISPLAY, 36), PALETTE["ink"], verdict[2] - verdict[0] - 52, gap=4)
    paragraph(
        draw,
        verdict[0] + 26,
        verdict[1] + 128,
...
def reviewer_focus(
    ...
    for value, label in stats:
        ...
        rounded_panel(draw, panel, fill=(245, 239, 228, 16), outline=(255, 255, 255, 28), radius=24, width=2, shadow=False)
    ...
    for item in ["README.md", "data/sweep_results/neurips_report.md", "paper/main.pdf", "docs/images/"]:
        ...
        rounded_panel(draw, panel, fill=(245, 239, 228, 12), outline=accent[:3] + (76,), radius=22, width=2, shadow=False)
        draw.text((panel[0] + 22, panel[1] + 24), item, font=font(DISPLAY, 24), fill=PALETTE["ink_soft"])
        path_y += 102
...
def level_one(draw: ImageDraw.ImageDraw, accent: tuple[int, int, int, int]) -> None:
    card(draw, (104, 620, 1030, 980), "Core loop", "A benchmark for sequential trial planning.", ["One run carries an agent through screening, follow-up, site allocation, and retention pressure over 180 simulated days."], accent)
    stat_box(draw, (104, 1018, 390, 1216), "180", "simulated days", accent)
