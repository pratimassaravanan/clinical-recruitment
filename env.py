...
# Set REWARD_DEBUG=1 to enable per-step reward breakdown logging.
# Useful for diagnosing reward=0 issues in GRPO training scripts.
_REWARD_DEBUG = os.environ.get("REWARD_DEBUG", "0").strip() == "1"
_reward_logger = logging.getLogger("clinical_env.reward")
if _REWARD_DEBUG and not _reward_logger.handlers:
    _h = logging.StreamHandler()
    _h.setFormatter(logging.Formatter("[REWARD_DEBUG] %(message)s"))
    _reward_logger.addHandler(_h)
    _reward_logger.setLevel(logging.DEBUG)
...
class ClinicalRecruitmentEnv:
    ...
    def _compute_reward(
        ...
        final_r = max(-0.5, min(0.99, r))

        if _REWARD_DEBUG:
            _reward_logger.debug(
                "step=%d task=%s action=%s hypothesis=%s | "
                "screen_success=%s enrolled=%s dropout=%s "
                "budget_ratio=%.3f enrolled_progress=%.3f "
                "in_curriculum=%s milestone_bonus=%.4f plan_bonus=%.4f "
                "memory_bonus=%.4f token_cost=%d token_penalty=%.4f "
                "hypothesis_bonus=%.4f step_pressure=%.4f consistency_penalty=%.4f "
                "=> RAW_r=%.4f FINAL_r=%.4f",
                self._step, self._task, action_type, hypothesis,
                outcome.get("screen_success"), outcome.get("enrolled"), outcome.get("dropout"),
                (self._budget_remaining / max(1, self._trace.get("budget", 1))),
                (self._enrolled / max(1, self._target)),
                in_curriculum, outcome.get("milestone_bonus", 0.0),
                outcome.get("plan_bonus", 0.0), outcome.get("memory_bonus", 0.0),
                int(outcome.get("token_cost", 0) or 0),
                min(0.05, int(outcome.get("token_cost", 0) or 0) / max(1, self._token_budget_total) * 1.2),
                self._hypothesis_bonus(hypothesis),
                0.005 * (self._step / self._max_steps),
                self._consistency_penalty(),
                r, final_r,
            )
