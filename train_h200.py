#!/usr/bin/env python3
"""Clinical Recruitment GRPO Training — H200 141GB — Qwen3-32B-4bit
...
print(f"GPU: {gpu} | VRAM: {vram:.1f}GB | CUDA: {torch.version.cuda} | PyTorch: {torch.__version__}")
...
LORA_ALPHA  = 32
NUM_GEN     = 16       # 16 generations per step — huge GRPO diversity
MAX_COMP    = 2048
BATCH       = 16       # big batch — H200 can handle it
GRAD_ACC    = 2
EPOCHS      = 3
LR          = 3e-6
NUM_PROMPTS = 192      # 3 epochs x 192 = lots of training signal
EVAL_STEPS  = 40
OUTPUT_DIR  = "grpo_clinical_recruitment_h200"
...
print(f"Config:  batch={BATCH} grad_acc={GRAD_ACC} num_gen={NUM_GEN} lr={LR} epochs={EPOCHS} prompts={NUM_PROMPTS}")
...
# ── Reward functions (dual-signature for TRL) ────────────────────────
...
if __name__ == "__main__":
    ...
    print(f"\n{sep}\nGRPO TRAINING on {gpu} ({vram:.0f}GB)\n{sep}")
    trainer.train()
    ...
    (RESULTS_DIR / "grpo_results_h200.json").write_text(
        json.dumps({"before": before, "after": after, "model": MODEL_NAME, "env_url": ENV_URL,
                    "gpu": gpu, "vram_gb": round(vram, 1), "config": {
                        "batch": BATCH, "grad_acc": GRAD_ACC, "num_gen": NUM_GEN,
                        "lr": LR, "epochs": EPOCHS, "lora_r": LORA_R, "prompts": NUM_PROMPTS,
                    }}, indent=2))
    print(f"\n{sep}\nDONE\n{sep}")
